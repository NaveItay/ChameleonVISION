...
class FieldDetection:
    ...
    # Import img keyboard arrows
    arrows_img_path = '/home/chameleonvision/Desktop/FinalProject/ChameleonVISION/assets/keyboard_arrow.png'
    arrows_img = cv2.imread(arrows_img_path)
    black = np.zeros((1274, 720, 3), dtype=np.uint8)
    # arrows_img = cv2.resize(field_img, (int(field_img.shape[1] / 2), int(field_img.shape[0] / 2)),
    #                        interpolation=cv2.INTER_AREA)
    # arrows_img[0:360, 0:655] = field_img
    ...
    def dynamic_field_thresholds(self, result_frame):
        ...
        if self.mode is not None:
            copy[495:720, 1055:1280] = self.arrows_img
